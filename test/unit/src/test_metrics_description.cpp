...
TEST(metrics_description, insert_parent_functor) {
    ...
    EXPECT_EQ(0, desc.at(std::numeric_limits<pcp::cluster_id_type>::min()).size());
    EXPECT_EQ(0, desc.at(std::numeric_limits<pcp::cluster_id_type>::max()).size());
    ...
    EXPECT_EQ(0, desc.at(std::numeric_limits<pcp::cluster_id_type>::min()).size());
    EXPECT_EQ(1, desc.at(std::numeric_limits<pcp::cluster_id_type>::max()).size());
...
TEST(metrics_description, insert_child_functor_1) {
    ...
    EXPECT_EQ(2, desc.size());         // desc didn't increase is size.
    EXPECT_EQ(0, desc.at(123).size()); // first cluster remained empty.
    EXPECT_EQ(2, desc.at(456).size()); // second cluster got two metrics.
